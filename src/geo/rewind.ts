...
function blurf(radius) {
  ...
  return (T, S, start, stop, step) => {
    ...
    for (let i = start, j = stop; i <= j; i += step) {
      ...
      T[i] =
        (sum2 + t * (S[Math.max(start, i - s1)] + S[Math.min(stop, i + s1)])) /
        w;
...
function polygonContains_default(polygon, point) {
    ...
    for (
      ...
      sum2.add(
        atan2(
          ...
          cosPhi03 * cosPhi1 + k * cos(absDelta),
        ),
      );
  ...
  return (angle < -epsilon || angle < epsilon && sum2 < -epsilon2) ^
    winding & 1;
}
  ...
  return abs(sinLambda0Lambda1) > epsilon
    ? atan(
      (sin(phi0) * (cosPhi1 = cos(phi1)) * sin(lambda1) -
        sin(phi1) * (cosPhi03 = cos(phi0)) * sin(lambda03)) /
...
function lengthPoint(lambda, phi) {
  ...
  var sinPhi = sin(phi),
    cosPhi = cos(phi),
    delta = abs(lambda - lambda02),
    cosDelta = cos(delta),
    sinDelta = sin(delta),
    x = cosPhi * sinDelta,
    y = cosPhi02 * sinPhi - sinPhi02 * cosPhi * cosDelta,
...
function contains_default(object2, point) {
  return (object2 && containsObjectType.hasOwnProperty(object2.type)
    ? containsObjectType[object2.type]
    : containsGeometry)(object2, point);
}
  ...
  do {
    ...
    phi -= delta = (phi *
        (1.007226 +
          phi2 *
            (0.015085 +
              phi4 * (-0.044475 + 0.028874 * phi2 - 5916e-6 * phi4))) - y) /
      (1.007226 +
        phi2 *
          (0.015085 * 3 +
            phi4 *
...
/**
 * Fixes the winding order of GeoJSON polygons so they follow the right-hand rule expected by `d3-geo` (exterior rings counter-clockwise, holes clockwise).
 *
 * Many data sources (including shapefiles exported by GIS tools) wind their exterior rings clockwise. Because `d3-geo` is a spherical library, it reads a clockwise exterior ring as covering the entire globe *except* that shape. This breaks anything that relies on polygon area or fill, most notably `fitExtent`/`fitSize`, which then collapse the projection's scale to nearly zero. Stroked outlines still look correct, which is why the problem often goes unnoticed until a projection is fitted to the raw polygons.
 *
 * Run the GeoJSON through `rewind` before fitting a projection to it to avoid this. The function returns a new object and does not mutate its input. Non-polygon geometries are returned unchanged.
 *
 * @param geojson The GeoJSON object to rewind (a `Feature`, `FeatureCollection`, or geometry).
 *
 * @returns A new GeoJSON object of the same type with corrected polygon winding.
 *
 * @example
 * ```ts
